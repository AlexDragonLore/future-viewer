...
Configuration is loaded via `IConfiguration` sections (`Jwt`, `AI`, `OpenAI`, `DeepSeek`, `Telegram`, `Admin`, `ConnectionStrings:Default`, `Cors:AllowedOrigins`). `AI:Provider` accepts `OpenAI`/`ChatGPT`/`GPT` or `DeepSeek`; provider-specific keys, models, and base URLs live in `OpenAI:*` and `DeepSeek:*`. The Telegram subsystem no-ops when `Telegram:BotToken` is empty; `FeedbackNotificationJob` and `TelegramPollingHostedService` exit early in that case. The webhook endpoint (`POST /api/telegram/webhook`) requires `Telegram:SecretToken` to be configured and matching the `X-Telegram-Bot-Api-Secret-Token` header — otherwise it returns 401. Secrets in dev live in user-secrets of `FutureViewer.Host`; in docker-compose they come from env vars `JWT_SECRET`, `AI_PROVIDER`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_BASE_URL`, `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`, `DEEPSEEK_BASE_URL`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME`, `TELEGRAM_WEBHOOK_URL`, `TELEGRAM_SECRET_TOKEN`, `TELEGRAM_SITE_URL`.
