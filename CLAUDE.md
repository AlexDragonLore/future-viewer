...
Future Viewer is a tarot-reading web app with AI-generated interpretations. Two deployable units talk over HTTP + JWT: a .NET 10 backend and a Vue 3 frontend, with PostgreSQL as the store and an OpenAI-compatible AI provider (`OpenAI`/`ChatGPT` or `DeepSeek`) as the interpreter.
...
The central reading flow: `POST /api/readings` → `ReadingService.CreateAsync` → draw from `CardDeckService` → `InterpretationService` (calls `IAIInterpreter`, implemented by `OpenAIInterpreter`) → persist via repository → `FeedbackService.ScheduleAsync` (best-effort, +24h) → return `ReadingResult`.
...
Configuration is loaded via `IConfiguration` sections (`Jwt`, `AI`, `OpenAI`, `DeepSeek`, `Telegram`, `Admin`, `Email`, `Support`, `ConnectionStrings:Default`, `Cors:AllowedOrigins`). `AI:Provider` accepts `OpenAI`/`ChatGPT`/`GPT` or `DeepSeek`; provider-specific keys, models, and base URLs live in `OpenAI:*` and `DeepSeek:*`. The Telegram subsystem no-ops when `Telegram:BotToken` is empty; `FeedbackNotificationJob` and `TelegramPollingHostedService` exit early in that case. Telegram updates are consumed via long polling only — `TelegramPollingHostedService` calls `DeleteWebhook` on startup so any pre-existing webhook is cleared before `ReceiveAsync` begins. The `Email` section drives `SmtpEmailSender`; when `Email:Host` is empty, sends are logged at `Information` (dev fallback) instead of being delivered. `Support:Email` is exposed verbatim by `GET /api/public/config` (anonymous) and rendered in the frontend footer via `usePublicConfigStore`. Secrets in dev live in user-secrets of `FutureViewer.Host`; in docker-compose they come from env vars `JWT_SECRET`, `AI_PROVIDER`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_BASE_URL`, `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`, `DEEPSEEK_BASE_URL`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_BOT_USERNAME`, `TELEGRAM_SITE_URL`, `SUPPORT_EMAIL`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_USERNAME`, `EMAIL_PASSWORD`, `EMAIL_FROM`, `EMAIL_USE_SSL`, `EMAIL_FRONTEND_URL`.
...
dotnet user-secrets set "AI:Provider" "OpenAI"       # or "DeepSeek"
dotnet user-secrets set "OpenAI:ApiKey" "sk-..."     # when AI:Provider=OpenAI
dotnet user-secrets set "DeepSeek:ApiKey" "<key>"    # when AI:Provider=DeepSeek
